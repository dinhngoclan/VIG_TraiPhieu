...
xls = pd.ExcelFile(r'D:\LIEUDANG\VIG\VIG2.xlsx')
VIG_mua = pd.read_excel(xls, 'VIG mua')
VIG_ban = pd.read_excel(xls, 'VIG ban')


mua = VIG_mua.groupby([r'NGÀY CHUYỂN TIỀN', r'Số HĐ', r'MÃ TP']).agg({'SỐ LƯỢNG': ['sum'], 'GIÁ GIAO DỊCH GROSS':['sum']})
mua.columns = ['SỐ LƯỢNG', r'GIÁ GIAO DỊCH GROSS']
mua = mua.reset_index()
mua['cumulative_TongSoLuongMua'] = mua.groupby([r'MÃ TP'])['SỐ LƯỢNG'].apply(lambda x: x.cumsum())
mua['GIÁ GIAO DỊCH GROSS'] = mua['GIÁ GIAO DỊCH GROSS'].astype(np.int64)
mua['cumulative_GiaGiaoDichGrossMua'] = mua.groupby([r'MÃ TP'])['GIÁ GIAO DỊCH GROSS'].apply(lambda x: x.cumsum())
mua[r'Số HD mua'] = mua[ r'Số HĐ'] + "_(" + mua[r'NGÀY CHUYỂN TIỀN'].map(str) + ")"
...
test = pd.merge(ban, mua, left_on=[r'MÃ TP'], right_on = [r'MÃ TP'], how='left')
test = test[(test[r'MÃ TP'] == test[r'MÃ TP'])&(test[r'Ngày giao dịch chuyển nhượng'] >= test[r'NGÀY CHUYỂN TIỀN'])]
...
offsetDict = {test.loc[0, r'Số HD mua']:test.loc[0,r'SỐ LƯỢNG_y']}
soLuongBan = test.loc[0,r'SỐ LƯỢNG_x']
hopDongBan = test.loc[0,r'Số HĐ_x']
for index, row in test.iterrows():
    hopDongMua = row[r'Số HD mua']
...
onlyBuyDf = test.copy()
onlyBuyDf = onlyBuyDf[onlyBuyDf[r'NGÀY CHUYỂN TIỀN'] < onlyBuyDf[r'Ngày giao dịch chuyển nhượng']]
onlyBuyDf[r'Ngày Giao Dịch'] = onlyBuyDf[r'NGÀY CHUYỂN TIỀN']
...
# onlySellDf = onlySellDf.groupby([r'Ngày Giao Dịch', r'MÃ TP', r'Số HD mua', r'SỐ LƯỢNG_y', r'GIÁ GIAO DỊCH GROSS_y']).agg({'ThucCon': ['min']})
...
for index, row in buySellMixedDf.iterrows():
    if row[r'Ngày giao dịch chuyển nhượng'] == row[r'NGÀY CHUYỂN TIỀN']: #ngày bán == ngày mua
...
buySellMixedDf = buySellMixedDf.groupby([r'Ngày Giao Dịch', r'MÃ TP', r'Số HD mua', r'SỐ LƯỢNG_y', r'GIÁ GIAO DỊCH GROSS_y', r'Loại Giao Dịch']).agg({'ThucCon': ['min']})
...
onlyBuyDf = onlyBuyDf.groupby([r'Ngày Giao Dịch', r'MÃ TP', r'Số HD mua', r'SỐ LƯỢNG_y', r'GIÁ GIAO DỊCH GROSS_y']).agg({'ThucCon': ['max']})
...
buySellDf = buySellDf.sort_values(by=[r'Ngày Giao Dịch', r'MÃ TP', r'Số HD mua'])
...
buySellDf.to_excel(r'D:\LIEUDANG\VIG\buySellDf.xlsx')
    	
test.to_excel(r'D:\LIEUDANG\VIG\output.xlsx')
